...
DBname = "storage" # "storact"
DBuser = "storage" #"sauser"
DBpwd = "storage" # "hello"
TableName = 'CensusData'
StagingTableName = "CensusDataStaging"
...
def createTable(conn):
	...
	with conn.cursor() as cursor:
		...
		print(f"Created {TableName} (logged).")
...
def createUnloggedTable(conn):
	...
	with conn.cursor() as cursor:
		...
		print(f"Created {StagingTableName} (unlogged).")


# Moved this code out of load() and created a function for modularity:
def create_PKs_and_index(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
            ALTER TABLE {TableName} ADD PRIMARY KEY (Year, CensusTract);
            CREATE INDEX idx_{TableName}_State ON {TableName}(State);
        """)
        print(f"Created PK on Year, CensusTract in {TableName}. Created index on State.")

def drop_PKs_and_index(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
            ALTER TABLE {TableName} DROP CONSTRAINT censusdata_pkey;
            DROP INDEX idx_{TableName}_State ON {TableName}(State);
        """)
        print(f"Dropped PK on Year, CensusTract in {TableName}. Dropped index on State.")
...
def stagingToMain(conn):
    with conn.cursor() as cursor:
        print(f"Loading {StagingTableName} into {TableName}.")
        start = time.perf_counter()
        ...
        elapsed = time.perf_counter() - start
        print(f'Inserted all from {StagingTableName} to {TableName}.')
        print(f'Finished Loading. Elapsed Time: {elapsed:0.4} seconds.')
...
def main():
    ...
    if CreateDB:
        ...
        # Now create logged table and transfer from unlogged table:
        createTable(conn)
    load(conn, cmdlist)

    # Insert data from staging table into main (logged) table:
    stagingToMain(conn)
    
    # Manually commit:
    commit_sql(conn)

    # Moved this out of load():
    #create_PKs_and_index(conn)

if __name__ == "__main__":
    main()


#python3 load_inserts.py -d acs2015_census_tract_data.csv -c -y 2015 
